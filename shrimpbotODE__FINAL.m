%%----SHRIMPBOT ODE MODELING - SOLVING FOR CURRENT-----%%
...
a = 80; % Amplitude
b = -40; % Minimum angle
c = 0.55; % Temporal asymmetry (% of cycle dedicated to the power stroke)
d_P_S = 0.8; % Power stroke wave shape
d_R_S = 1.1; % Recovery stroke wave shape
...
I_iso1 = @(t,C_D_c) ( (I_c + M*l_c^2) * (theta1_dot_dot(t))*(pi/180) + ...
            ...
            ((C_D_r*(((w3-w2)/(h3-h2))*((4*h3^5-5*h2*h3^4+h2^5)/(20))+((w2/4)*(h3^4-h2^4)))))+... Region 3
            ((C_D_c*(((w4-w3)/(h4-h3))*((4*h4^5-5*h3*h4^4+h3^5)/(20))+((w3/4)*(h4^4-h3^4)))))+... Region 4
            ((C_D_c*(((w5-w4)/(h5-h4))*((4*h5^5-5*h4*h5^4+h4^5)/(20))+((w4/4)*(h5^4-h4^4)))))+... Region 5
            ((C_D_c*(((w6-w5)/(h6-h5))*((4*h6^5-5*h5*h6^4+h5^5)/(20))+((w5/4)*(h6^4-h5^4)))))...  Region 6
            )...
            )... 
...
ylabel("Solenoid Current (A)", 'Interpreter', 'Latex', 'FontSize', 16)
title("Solenoid Current to Produce Metachronal Swimming", 'Interpreter', 'Latex', 'FontSize', 18);
%grid on;

% figure
% hold on
% fplot(theta1, [0 c], 'LineWidth', 2)
% fplot(theta2, [c 1], 'LineWidth', 2)
% title("Shrimp Leg Metachronal Swimming Function", 'Interpreter', 'Latex', 'FontSize', 18);
% xlabel("Time (s)", 'Interpreter', 'Latex', 'FontSize', 16)
% ylabel("Leg Angle with Vertical (deg)", 'Interpreter', 'Latex', 'FontSize', 16)
